...
class Magehit_Sociallogin_Model_Twitter_Client
{
    const REDIRECT_URI_ROUTE = 'mhsociallogin/twitter/connect';
    const REQUEST_TOKEN_URI_ROUTE = 'mhsociallogin/twitter/request';

    const OAUTH_URI = 'https://api.twitter.com/oauth';
    const OAUTH2_SERVICE_URI = 'https://api.twitter.com/1.1';    

    const XML_PATH_ENABLED = 'magehit/magehit_sociallogin_twitter/enabled';
    const XML_PATH_CLIENT_ID = 'magehit/magehit_sociallogin_twitter/client_id';
    const XML_PATH_CLIENT_SECRET = 'magehit/magehit_sociallogin_twitter/client_secret';
    ...
    protected function _httpRequest($url, $method = 'GET', $params = array())
    {
        ...
        if($response->isError()) {
            ...
            if(($status == 400 || $status == 401 || $status == 429)) {
                ...
                throw new Magehit_Sociallogin_TwitterOAuthException($message);
            } else {
                $message = sprintf(
                    Mage::helper('sociallogin')
...
class Magehit_Sociallogin_TwitterOAuthException extends Exception
{}
